...
- `docs/schedule-rotation.md` - правила замены расписания mid-rotation (защита текущей ротации), авто-отмена пропущенных вылетов, one-way рейсы между хабами и frontend-анализатор готовности недельной сетки.
- `docs/fuel-storage.md` - топливное хранилище авиакомпании: закупка впрок, идемпотентное списание при сеттлменте рейсов, спот-добор, контракт `/fuel/storage` и UI (панель топлива + топбар).
