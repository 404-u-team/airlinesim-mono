...
- `src/config` - чтение browser-facing `VITE_BFF_URL` и `VITE_SOCKET_URL`.
...
Адрес HTTP API для browser-клиентов задается через `.env`: `VITE_BFF_URL`. SDK ходит в BFF, а BFF уже обращается к backend через `BFF_BACKEND_BASE_URL`. Для socket endpoint используется `VITE_SOCKET_URL`.
