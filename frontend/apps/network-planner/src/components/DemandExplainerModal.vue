...
const rows = computed<Array<{ highlight?: boolean; label: string; value: string }>>(() => {
  ...
  return [
    ...
    { label: props.t("demand.sameCountry"), value: data.sameCountry ? `${props.t("demand.yes")} (×${formatFactor(data.domesticMultiplier)})` : props.t("demand.no") },
    { label: props.t("demand.shortHaul"), value: `×${formatFactor(data.shortHaulFactor)}` },
