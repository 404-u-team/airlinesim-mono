...
type RegionLinkRepository interface {
	CreateRegionLink(ctx context.Context, payload *operationspb.CreateRegionLinkRequest) (uuid.UUID, error)
	ChangeRegionLink(ctx context.Context, payload *operationspb.ChangeRegionLinkRequest) (bool, error)
	ListRegionLinks(ctx context.Context) ([]*operationspb.RegionLink, error)
...
func (r *regionLinkRepository) ListRegionLinks(ctx context.Context) ([]*operationspb.RegionLink, error) {
	query := `
		SELECT
			id::text,
			region_a::text,
	...
	regionLinks := make([]*operationspb.RegionLink, 0)
	for rows.Next() {
		var regionLink operationspb.RegionLink
		if err := rows.Scan(
			&regionLink.Id,
			&regionLink.RegionA,
			&regionLink.RegionB,
