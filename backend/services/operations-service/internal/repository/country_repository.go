...
type CountryRepository interface {
	CreateCountry(ctx context.Context, payload *operationspb.CreateCountryRequest) (uuid.UUID, error)
	ChangeCountry(ctx context.Context, payload *operationspb.ChangeCountryRequest) (bool, error)
	ListCountries(ctx context.Context) ([]*operationspb.Country, error)
...
func (r *countryRepository) CreateCountry(ctx context.Context, payload *operationspb.CreateCountryRequest) (uuid.UUID, error) {
	query := `
		INSERT INTO country (
...
func (r *countryRepository) ChangeCountry(ctx context.Context, payload *operationspb.ChangeCountryRequest) (bool, error) {
...
func (r *countryRepository) ListCountries(ctx context.Context) ([]*operationspb.Country, error) {
	query := `
		SELECT
			id::text,
			iso,
	...
	countries := make([]*operationspb.Country, 0)
	for rows.Next() {
		var country operationspb.Country
		if err := rows.Scan(
			&country.Id,
			&country.Iso,
			&country.LocalName,
