...
type AirportRepository interface {
	CreateAirport(ctx context.Context, payload *operationspb.CreateAirportRequest) (uuid.UUID, error)
	ChangeAirport(ctx context.Context, payload *operationspb.ChangeAirportRequest) (bool, error)
	ListAirports(ctx context.Context) ([]*operationspb.Airport, error)
...
func (r *airportRepository) CreateAirport(ctx context.Context, payload *operationspb.CreateAirportRequest) (uuid.UUID, error) {
	query := `
		INSERT INTO airport (
...
func (r *airportRepository) ChangeAirport(ctx context.Context, payload *operationspb.ChangeAirportRequest) (bool, error) {
...
func (r *airportRepository) ListAirports(ctx context.Context) ([]*operationspb.Airport, error) {
	query := `
		SELECT
			id::text,
			COALESCE(icao_code, ''),
	...
	airports := make([]*operationspb.Airport, 0)
	for rows.Next() {
		var airport operationspb.Airport
		if err := rows.Scan(
			&airport.Id,
			&airport.IcaoCode,
			&airport.IataCode,
