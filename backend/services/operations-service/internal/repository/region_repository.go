...
type RegionRepository interface {
	CreateRegion(ctx context.Context, payload *operationspb.CreateRegionRequest) (uuid.UUID, error)
	ChangeRegion(ctx context.Context, payload *operationspb.ChangeRegionRequest) (bool, error)
	ListRegions(ctx context.Context) ([]*operationspb.Region, error)
...
func (r *regionRepository) CreateRegion(ctx context.Context, payload *operationspb.CreateRegionRequest) (uuid.UUID, error) {
	query := `
		INSERT INTO region (
...
func (r *regionRepository) ChangeRegion(ctx context.Context, payload *operationspb.ChangeRegionRequest) (bool, error) {
...
func (r *regionRepository) ListRegions(ctx context.Context) ([]*operationspb.Region, error) {
	query := `
		SELECT
			id::text,
			local_code,
	...
	regions := make([]*operationspb.Region, 0)
	for rows.Next() {
		var region operationspb.Region
		if err := rows.Scan(
			&region.Id,
			&region.LocalCode,
			&region.LocalName,
